...
def evaluate_eight_class(model_path, test_dir, batch_size, output_dir, target_height=240, target_width=320):
    """
    Evaluate eight-class classification: classes 0 to 7.
    """
    print("[INFO] Evaluating eight-class classification...")
    ...
    try:
        ...
        # Create data generator for eight-class mode using the new DataGenerator8Classes
        test_gen = DataGenerator8Classes(
            data_dir=test_dir,
            batch_size=batch_size,
            shuffle=False,
            balance_classes=False,
        ...
        if len(test_gen.filepaths) == 0:
            print("[ERROR] No test data found for eight-class evaluation")
            return {
                'accuracy': 0.0,
                'class_report': None,
                'confusion_matrix': None,
        ...
        # Save detailed results
        results_path = os.path.join(output_dir, f'eight_class_results_{datetime.now().strftime("%Y%m%d_%H%M%S")}.txt')
        with open(results_path, 'w') as f:
            f.write(f"Eight-Class Classification Results\n")
            f.write(f"==========================================\n\n")
            f.write(f"Test Date: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
            f.write(f"Model: {model_path}\n")
            f.write(f"Test Data: {test_dir}\n")
...
def test_model(test_dir, model_path, batch_size, output_base_dir="./result", method_name="unknown", 
    ...
    Supports binary, three-class, and eight-class classification modes.
    """
    try:
        # Determine classification mode and create appropriate directory
        if eight_class_mode:
            mode_name = "eight_class"
            test_func = evaluate_eight_class
        elif three_class_mode:
            mode_name = "three_class"
            test_func = evaluate_three_class
        else:
            mode_name = "binary"
            test_func = evaluate_all_leak_vs_no_leak
            
        print(f"\n{'='*70}")
        print(f"{mode_name.upper()} TESTING FOR {method_name}")
        print(f"{'='*70}")
        ...
        # Create mode-specific directory with resolution info
        mode_dir = os.path.join(output_dir, f"{mode_name}_test")
        os.makedirs(mode_dir, exist_ok=True)
        
        dimension_str = f"{target_height}x{target_width}"
        test_dir_with_info = os.path.join(mode_dir, dimension_str)
        os.makedirs(test_dir_with_info, exist_ok=True)
        
        # Save configuration info
        config_file = os.path.join(test_dir_with_info, f"test_config_{datetime.now().strftime('%Y-%m-%d')}.txt")
        with open(config_file, 'w') as f:
            ...
            f.write(f"Mode: {mode_name.capitalize()} Classification\n")
            if distance_filter:
        ...
        # Run evaluation for the selected mode
        results = test_func(model_path, test_dir, batch_size, test_dir_with_info, 
                         target_height, target_width)
        
        # If it's three-class mode and no distance filter is applied,
        # also evaluate for specific distances if files exist
        if three_class_mode and not distance_filter:
            # Try 4.6m distance evaluation
            try:
                print(f"\n{'='*70}")
                print(f"THREE-CLASS TESTING FOR {method_name} AT 4.6m DISTANCE")
                print(f"{'='*70}")
                evaluate_three_class_distance_46(model_path, test_dir, batch_size, test_dir_with_info,
                                            target_height, target_width)
            except Exception as e:
                print(f"[WARNING] Distance 4.6m evaluation skipped: {e}")
                
            # Try 6.9m distance evaluation
            try:
                print(f"\n{'='*70}")
                print(f"THREE-CLASS TESTING FOR {method_name} AT 6.9m DISTANCE")
                print(f"{'='*70}")
                evaluate_three_class_distance_69(model_path, test_dir, batch_size, test_dir_with_info,
                                            target_height, target_width)
            except Exception as e:
                print(f"[WARNING] Distance 6.9m evaluation skipped: {e}")
