"""
Running Average background subtractor for efficient processing.
...
def runavg_process_video(
    ...
    Using Running Average background subtractor for efficient processing.
    - For class 0 only, use the Excel 'Start Time for Leak 0 Video Clip'.
    - For all classes, remove the first 15s and last 5s before saving.
    - Continuous background subtraction with a 210-frame rolling buffer.
    - If the filename has '_s2', we do a train/val split; if '_s1', we save everything to test.
    ...
    for class_label in range(total_segments):
        ...
        while current_frame_index < chunk_end_frame:
            ...
            # Running Average Background Subtraction using OpenCV functions
            if first_frame:
                # Initialize background with first frame
                background = gray.copy().astype(np.float32)
                fg_mask = np.zeros_like(gray, dtype=np.uint8)
                first_frame = False
            else:
                # Update background model with running average
                cv2.accumulateWeighted(gray.astype(np.float32), background, alpha)
                
                # Convert background to uint8 for comparison
                background_uint8 = cv2.convertScaleAbs(background)
                
                # Compute foreground mask
                fg_mask = cv2.absdiff(gray, background_uint8)
